            ...
            builder.setTitle("Error").setMessage("Can not move " + appName + ": Remove installed updates first.")
                    .setPositiveButton("Remove updates", new DialogInterface.OnClickListener() {
                        public void onClick(final DialogInterface dialog, int id) {
                            try {
                                ap.activity.startActivity(new Intent(Intent.ACTION_DELETE, Uri
                                        .parse("package:" + app.packageName)));
                                dialog.dismiss();
                            } catch (Exception e) {
                            }
                        }
                    ...
                    try {
                        ...
                        if (!alreadySys) {
                            if (app.sourceDir.endsWith("/pkg.apk")) {
                                newFile = MoverActivity.SYSTEM_APP_FOLDER + app.packageName + "-asec.apk";
                                ...
                                newFile = app.sourceDir.replace("/data/app/", MoverActivity.SYSTEM_APP_FOLDER);
                                mvcmd = "busybox mv " + app.sourceDir + " " + MoverActivity.SYSTEM_APP_FOLDER;
                            }
                        } else {
                            if (app.sourceDir.endsWith("/pkg.apk")) {
                                ...
                                newFile = app.sourceDir.replace(MoverActivity.SYSTEM_APP_FOLDER, "/data/app/");
